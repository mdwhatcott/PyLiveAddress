...
class LiveAddressService(object):
    ...
    def __init__(self, suggestions=1, sandbox=False, logging=False):
    
        self.sandbox = sandbox
        if logging: self._enable_logging()
        self.client = Client(SERVICE_URL)
        self.suggestions = 1 # TODO: allow multiple suggestions...
    ...
    def verify_address(self, address):
        ...
        if not result.Success:
            return LiveAddressResponse(success=False, qualified_addresses=[address], message=QAD_DOWN + result.Message)

        if len(result.Addresses.ArrayOfAddressResponse) < 1:
            return LiveAddressResponse(success=False, qualified_addresses=[address], message=BAD_ADDRESS)
 
        return LiveAddressResponse(success=True, qualified_addresses=self._package_result_addresses(result))
    ...
    def _package_result_addresses(self, result):
        ...
        addresses = [result.Addresses.ArrayOfAddressResponse[0].AddressResponse[0]]
        return [QualifiedAddress(a['Street'], a['Street2'], a['City'], a['State'], a['ZipCode']) for a in addresses]
...
class LiveAddressResponse(object):
    ...
    def __init__(self, success=False, qualified_addresses=None, message=''):
    ...
    def __repr__(self):
        ...
        rep += TAB + 'success={0},'.format(self.success)
...
if __name__ == '__main__':
    ...
    address = InputAddress(street1='3214 N University Ave #221', city='provo', state='UT')
    ...
    print
    print LiveAddressService(sandbox=True).verify_address(address)
    print 
